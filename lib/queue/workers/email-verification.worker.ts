    ...
    try {
      ...
      for (const c of candidates) {
        ...
        if (isValid || isCatchAllAccepted) {
          ...
          // Also upsert verified account if oauth email
          verifiedCount++;
        }
      }
      ...
      logger.error({ sessionId, err }, 'Email verification worker failed');
      await PipelineJob.findOneAndUpdate(
        { sessionId, jobType: 'email_verification' },
        { status: 'failed', completedAt: new Date(), errorMessage: msg }
      );
      await publishSSEEvent(sessionId, 'job_failed', { stage: 'email_verification', error: msg });

      // Ensure VisitorSession is still marked complete to stop visual spin
      await VisitorSession.findOneAndUpdate(
        { sessionId },
        {
          pipelineStatus: 'complete',
      ...
      await publishSSEEvent(sessionId, 'pipeline_complete', { verifiedEmailCount: 0 });
    }
  },
