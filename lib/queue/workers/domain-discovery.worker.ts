    ...
    try {
      ...
      if (!domain) {
        ...
        await writeAuditLog({
          ...
          actor: 'worker',
          details: { skipped: true, reason: 'no_domain' },
        });
      ...
      const msg = err instanceof Error ? err.message : 'Domain discovery failed';
      logger.error({ sessionId, err }, 'Domain discovery worker failed');
      await PipelineJob.findOneAndUpdate(
        { sessionId, jobType: 'domain_discovery' },
        { status: 'failed', completedAt: new Date(), errorMessage: msg }
      );
      await publishSSEEvent(sessionId, 'job_failed', { stage: 'domain_discovery', error: msg });
